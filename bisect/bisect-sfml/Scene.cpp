...
void Scene::BisectByLongest( int index )
{
	...
	float2 tangent = linalg::normalize( polygon.vertices[(maxIndex + 1) % n] - polygon.vertices[maxIndex] );
	float2 normal = linalg::normalize( inner_normal(
		polygon.vertices[maxIndex],
		polygon.vertices[(maxIndex + 1) % n],
		polygon.vertices[(maxIndex + 2) % n] ) );

	float randrad = (random() - 0.5f) * 3.1415f * 0.25f;
	float2 direction = linalg::normalize( cosf( randrad ) * normal + sinf( randrad ) * tangent );
